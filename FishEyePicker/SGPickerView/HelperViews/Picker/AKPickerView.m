...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.delegate respondsToSelector:@selector(pickerView:marginForItem:)]) {
      CGFloat originalY = cell.subTitleLabel.frame.origin.y;
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if ([self.dataSource respondsToSelector:@selector(pickerView:titleForItem:)]) {
		NSString *title = [self.dataSource pickerView:self titleForItem:indexPath.item];
    NSString *subTitle = [self.dataSource pickerView:self subTitleForItem:indexPath.item];
    CGFloat maxWidth = MAX([self sizeForString:title].width, [self sizeForString:subTitle].width);
		size.width += maxWidth;
		if ([self.delegate respondsToSelector:@selector(pickerView:marginForItem:)]) {
			CGSize margin = [self.delegate pickerView:self marginForItem:indexPath.item];
...
- (void)initialize
{
  ...
  CGRect labelRect = CGRectMake(self.contentView.bounds.origin.x, self.contentView.bounds.origin.y, self.contentView.bounds.size.width, self.contentView.bounds.size.height / 2);
  ...
  // Subtitle label
  CGRect subTitleLabelRect = labelRect;
  subTitleLabelRect.size.height = self.contentView.bounds.size.height / 2;
  subTitleLabelRect.origin.y = CGRectGetMaxY(labelRect);
  self.subTitleLabel = [[UILabel alloc] initWithFrame:subTitleLabelRect];
  ...
  self.subTitleLabel.font = [UIFont systemFontOfSize:[UIFont systemFontSize]];
...
- (void)setSelected:(BOOL)selected
{
  ...
  [self.subTitleLabel.layer addAnimation:transition forKey:nil];

	self.label.font = self.selected ? self.highlightedFont : self.font;
  self.subTitleLabel.font = self.selected ? self.highlightedFont : self.font;
}
